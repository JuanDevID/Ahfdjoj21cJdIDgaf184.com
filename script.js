...
window.addEventListener('DOMContentLoaded', function() {
  ...
  function showWelcomeAnim() {
    ...
    // Dialogue lines
    const dialogue = [
      {el: ai1, text: "👋 Hello, I'm Sylpha-AI. Welcome to my portfolio!"},
...
function startBgRGB() {
  ...
  chatbotForm.onsubmit = async (e) => {
    ...
    appendMessage('Typing...', 'bot-msg', true);
    const botResponse = await fetch('/chat', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: userMsg }),
    }).then(res => res.json()).catch(() => ({ response: 'Sorry, error occurred.' }));

    const typingMsg = chatbotMessages.querySelector('.bot-msg.typing');
    if (typingMsg) typingMsg.remove();

    appendMessage(botResponse.response || 'No reply.', 'bot-msg');
  };
  ...
  window.addEventListener("load", () => {
    const intro = document.getElementById("juanai-intro");
    const loader = document.getElementById("loader");
  
    setTimeout(() => {
      intro.style.display = "none";
    }, 5000); // Tampilkan intro selama 5 detik
  
    // Loader delay jika ingin lebih sinematik
    setTimeout(() => {
  ...
  function toggleAIPanel() {
    ...
    document.getElementById('loader').style.display = 'flex'; // Tampilkan loader saat panel chat dimunculkan
    setTimeout(() => {
      document.getElementById('loader').style.display = 'none'; // Sembunyikan loader setelah 2 detik
    }, 2000);
  }
  
  // Fungsi untuk menutup panel chat AI
    ...
    if (event.key === "Enter") {
      ...
      if (message) {
        ...
        setTimeout(() => {
          ...
          aiMessage.innerHTML = '<span>AI: I am processing your request...</span>';
